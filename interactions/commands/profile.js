...
module.exports = {
  ...
  async execute(interaction) {
    ...
    switch (_subcommand) {
      case `user`:
        console.log("called user subcommand");
        break;
      case `update`:
        update(interaction);
        break;
...
async function update(interaction) {
  ...
  if (riotIDFromDB === riotIDFromRiot) {
    const embed = new EmbedBuilder({
      author: { name: `Error` },
      description: "Your nickname is the same as your Valorant IGN!",
      color: 0xe92929,
      footer: {
        text: "Profile - Update",
      },
    ...
    const embed = new EmbedBuilder({
      author: { name: `Success` },
      description: "Your Riot tagline has been updated in the VDC database!",
      color: 0x008000,
      footer: {
        text: "Profile - Update",
      },
