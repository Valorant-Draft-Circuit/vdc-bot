...
const { Tier, ContractStatus, TransactionType } = require("@prisma/client");
...
async function confirmCut(/** @type ButtonInteraction */ interaction) {
	...
	if (playerData.Status.contractStatus === ContractStatus.ACTIVE_SUB && playerData.team !== null) {
		await restorePairedSubbedOutPlayer(playerData.id, playerData.team);
	}
	...
	try {
		const dmEmbed = new EmbedBuilder({
			description: `Unfortunately, you've been cut from ${franchise.name}, but that's not the end! If you want to stay involved & showcase your talent to other teams-- as well as showing your old team that they would've been way better off with you on it... Join the FA Hub!`,
